...
export default function DenyRequestModel({ open, setOpen ,webid,request,datas}) {
    ...
    const updatesatus = async () => {
      const result = await UPDATEEDIT({
        "editRequest": 0,
        "editRequestRejectedBy": user.name,
        // "rejectBy": user.name,
        // "status": "Reject",
        // "approval_date": date
      },webid)
      ...
      Toast.fire({
        icon: 'success',
        title:'<span style="font-size: 14px">Editing access has been deny</span>',
      })
            ...
            <div id="modal-desc" textColor="text.tertiary">
              <div className="alloweddit ">
                <div className="peoplelabin ">
                  <h2 style={{ fontSize: '22px',color:'#223F63',fonWeight:'600',fontFamily: 'Inter' }}>Deny Editing?</h2>
                  <div className="">
              ...
              <button className="modelcancel mb-0 d-flex align-items-center justify-content-center me-3" style={{border:'1px solid'}} onClick={handlecancel} >Cancel</button>
                <button className="modelbottomdeny mb-0 d-flex align-items-center justify-content-center"  onClick={updatesatus}>Deny</button>
              </div>
            </div>
          </Sheet>
        </Modal>
