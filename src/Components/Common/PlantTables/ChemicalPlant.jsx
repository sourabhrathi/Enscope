...
const ChemicalPlant = ({ month, YearData, CreatOn, year, manthFilter }) => {
                    ...
                    <tbody>
                        {rowData.map((item, index) => (
                            <tr key={index}>
                                ...
                                <td> {item?.quantity} <span style={{ color: '#A2A2A2', fontSize: '14px' }}>{item?.unit}</span> </td>
                            </tr>
                        ))}
                    </tbody>
                </table>
